...
namespace shift_lib
{
	...
	//**********************************************************************
	void print_progressbar(double percentage, std::ostream & out_local, int length, string symbol)
	// Print out a progress bar with the given percentage. Use a negative value to reset the progress bar.
	{
	  ...
	  if (status==0)
	  {
		out_local << "\r";
		out_local << "[";
		for (int i=1; i<=length; i++) out_local << " ";
		out_local << "]";
		out_local << "\r";
		out_local << "[";
	  }
	  ...
	  for (int i=previous_stop; i<stop; i++) out_local << symbol;
	  if (previous_stop<stop) previous_stop=stop;

	  // simulate a rotating bar
	  if (status==0) out_local << "-";
	  switch (status)
	  {
		case 1: out_local << "\\"; break;
		case 2: out_local << "|"; break;
		case 3: out_local << "/"; break;
		case 4: out_local << "-"; break;
	  }
	  out_local << "\b";
	  status++;
