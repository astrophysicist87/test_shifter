  ...
  for (int i = 0; i < np; i++)
  {
    ...
    for (int j = i+1; j < np; j++)
    {
      ...
      double tmp = exp(-0.25*q2*R*R);
			// cout << "CHECK COMPARISON: " << BE_distance[UTindexer(pair.ID1, pair.ID2, total_n)] << "  " << tmp << "\n";
if (abs(BE_distance[UTindexer(pair.ID1, pair.ID2, total_n)]-tmp) > 1e-10)
{
  ...
  cout << "  " << BE_distance.at(UTindexer(pair.ID1, pair.ID2, total_n)) << "  ";
  cout << abs(tmp-BE_distance.at(UTindexer(pair.ID1, pair.ID2, total_n))) << "\n\n";
